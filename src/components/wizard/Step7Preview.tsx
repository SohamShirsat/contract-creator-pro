  ...
  return (
    <div className="cc-card">
      <button
        ...
        style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", background: "none", border: 0, cursor: "pointer", padding: 0, marginBottom: open ? 16 : 0 }}
      >
        <h3 style={{ fontSize: 16, fontWeight: 600 }}>{title}</h3>
        <span style={{ fontSize: 16, color: "var(--color-muted-foreground)" }}>{open ? "▾" : "▸"}</span>
      </button>
      {open && children}
    </div>
  );
...
export function Step7Preview() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <Section title="Setup preview">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <KV k="Contract name" v={state.contractName} />
          <KV k="Hotel name" v={state.hotelName} />
      ...
      <Section title="Season & Room pricing">
        {state.seasons.map((s) => (
          <div key={s.id} style={{ marginBottom: 24 }}>
            <div style={{ fontWeight: 600, marginBottom: 4 }}>📅 {s.name}</div>
            <div style={{ fontSize: 12, color: "var(--color-muted-foreground)", marginBottom: 12 }}>{s.from} → {s.to}</div>
            <table className="cc-table cc-table-compact">
              <thead>
                <tr>
                  <th>Room & Meal</th><th>Base</th><th>1P</th><th>2P</th><th>3P</th>
                </tr>
              </thead>
              <tbody>
                {state.rooms.map((r) => (
          ...
          {state.addons.map((a) => (
            <div key={a.id} style={{ padding: 16, background: "var(--color-muted)", borderRadius: 8 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, fontSize: 13 }}>
                ...
                {a.childPrice && <div><strong>Child:</strong> ₹{a.childPrice}</div>}
      ...
      <Section title="Policies">
        <div style={{ display: "flex", flexDirection: "column", gap: 12, fontSize: 13 }}>
          <KV k="No show penalty" v={`${state.noShowPenalty} ${state.noShowUnit}`} />
          ...
          <div>
            ...
            <table className="cc-table cc-table-compact">
              <thead><tr><th>Condition</th><th>When</th><th>Penalty</th><th>Fees</th></tr></thead>
              <tbody>{state.cancelBefore.map((r) => <tr key={r.id}><td>{r.condition}</td><td>{r.appliesWhen}</td><td>{r.penalty}%</td><td>{r.processingFees}%</td></tr>)}</tbody>
            </table>
          </div>
          <div>
            <div style={{ fontSize: 12, color: "var(--color-muted-foreground)", marginBottom: 4 }}>Cancellation (after check-in)</div>
            <table className="cc-table cc-table-compact">
              <thead><tr><th>Condition</th><th>Penalty</th><th>Fees</th></tr></thead>
              <tbody>{state.cancelAfter.map((r) => <tr key={r.id}><td>{r.condition}</td><td>{r.penalty}%</td><td>{r.processingFees}%</td></tr>)}</tbody>
            </table>
          </div>
          <div>
      ...
      <Section title="Contract validity">
  ...
  return (
    <>
      <tr style={{ background: "var(--color-muted)" }}>
        <td colSpan={5} style={{ height: 40, fontWeight: 600 }}>{roomName}</td>
      </tr>
