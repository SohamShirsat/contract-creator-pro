...
export function Step4Policies() {
  ...
  const [draftCancel, setDraftCancel] = useState<CancelRule>({ id: "", condition: "", appliesWhen: "", penalty: "", processingFees: "" });
  const [draftMinlos, setDraftMinlos] = useState<MinLOS>({ id: "", restrictionType: "Minimum stay", appliesTo: "", minNights: "", roomType: "All rooms" });
  ...
  const saveCancel = () => {
    ...
    setDraftCancel({ id: "", condition: "", appliesWhen: "", penalty: "", processingFees: "" });
  };

  const cancelTable = (rules: CancelRule[], key: "cancelBefore" | "cancelAfter", conditions: string[]) => (
    <table className="cc-table">
      <thead>
        <tr>
          <th>Condition</th><th>Applies when</th><th>Penalty</th><th>Processing fees</th><th></th>
        </tr>
      </thead>
      <tbody>
        {rules.map((r) => (
          <tr key={r.id}>
            <td>
              <select className="cc-input" value={r.condition} onChange={(e) => updateCancel(key, r.id, { condition: e.target.value })}>
                <option value="">Select</option>
                {conditions.map((c) => <option key={c}>{c}</option>)}
              </select>
            </td>
            <td><input className="cc-input" value={r.appliesWhen} onChange={(e) => updateCancel(key, r.id, { appliesWhen: e.target.value })} /></td>
            <td><input className="cc-input" value={r.penalty} onChange={(e) => updateCancel(key, r.id, { penalty: e.target.value })} /></td>
            <td><input className="cc-input" value={r.processingFees} onChange={(e) => updateCancel(key, r.id, { processingFees: e.target.value })} /></td>
            <td><button className="cc-icon-btn" onClick={() => removeCancel(key, r.id)}>✕</button></td>
          </tr>
        ))}
      </tbody>
    </table>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Card title="No show policy">
        <div style={{ display: "flex", gap: 12, alignItems: "flex-end", marginBottom: 12 }}>
          <div style={{ flex: 1, maxWidth: 240 }}>
            <label className="cc-label">No show penalty</label>
            <input className="cc-input" value={state.noShowPenalty} onChange={(e) => setState({ ...state, noShowPenalty: e.target.value })} />
          </div>
          <div style={{ width: 100 }}>
            <select className="cc-input" value={state.noShowUnit} onChange={(e) => setState({ ...state, noShowUnit: e.target.value as "%" | "₹" | "Nights" })}>
              <option>%</option><option>₹</option><option>Nights</option>
            </select>
          </div>
        </div>
      ...
      {/* Cancellation before */}
      <Card title="Before check-in — Cancellation policy" action={
        <button className="cc-btn cc-btn-outline" onClick={() => setCancelModal({ open: true, type: "before" })}>+ Add rule</button>
      }>
        {cancelTable(state.cancelBefore, "cancelBefore", ["Before Check-in", "Partial cancellation"])}
      </Card>

      {/* Cancellation after */}
      <Card title="After check-in — Cancellation policy" action={
        <button className="cc-btn cc-btn-outline" onClick={() => setCancelModal({ open: true, type: "after" })}>+ Add rule</button>
      }>
        {cancelTable(state.cancelAfter, "cancelAfter", ["Early departure", "Partial cancellation"])}
      </Card>
      ...
      <Card title="Payment policy">
        <div style={{ display: "flex", gap: 24 }}>
          {(["Pay later at month end", "Collect in installments"] as const).map((v) => (
            <label key={v} className="cc-radio-row">
          ...
          <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 12 }}>
            {state.installments.map((inst, i) => (
              <div key={inst.id} style={{ display: "flex", gap: 12, alignItems: "center" }}>
                <span style={{ fontSize: 13, width: 100 }}>Installment {i + 1}</span>
                <input className="cc-input" placeholder="Amount %" value={inst.amount} onChange={(e) => setState((s) => ({ ...s, installments: s.installments.map((x) => x.id === inst.id ? { ...x, amount: e.target.value } : x) }))} />
                <input type="date" className="cc-input" value={inst.date} onChange={(e) => setState((s) => ({ ...s, installments: s.installments.map((x) => x.id === inst.id ? { ...x, date: e.target.value } : x) }))} />
              </div>
            ))}
          </div>
        )}
      ...
      {/* Blackouts */}
      <Card title="Stop Sale / Blackouts / Soldout Dates">
        <p style={{ fontSize: 12, color: "var(--color-muted-foreground)", marginBottom: 12 }}>
          Static rates will not be valid either use dynamic pricing from PMS or call hotel directly.
        </p>
        ...
        <h4 style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>Blackouts</h4>
        ...
        <h4 style={{ fontSize: 14, fontWeight: 600, marginTop: 24, marginBottom: 8 }}>Stop Sale</h4>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <Card title="Minimum Length of Stay (MinLOS)" action={
        <button className="cc-btn cc-btn-outline" onClick={() => { setDraftMinlos({ id: "", restrictionType: "Minimum stay", appliesTo: "", minNights: "", roomType: "All rooms" }); setMinlosModal(true); }}>+ Add</button>
      }>
        <table className="cc-table cc-table-compact">
          <thead><tr><th>Restriction type</th><th>Applies to</th><th>Minimum nights</th><th>Room type</th><th></th></tr></thead>
          <tbody>
            {state.minLOS.map((m) => (
              <tr key={m.id}>
                <td>{m.restrictionType}</td><td>{m.appliesTo}</td><td>{m.minNights}</td><td>{m.roomType}</td>
      ...
      <Card title="FOC Policy">
        ...
        {state.focPolicy === "Applicable" && (
          <>
            ...
            <button className="cc-btn cc-btn-outline" onClick={() => setState((s) => ({ ...s, focTiers: [...s.focTiers, { id: uid(), from: "", to: "", roomType: s.rooms[0]?.name || "", mealPlan: s.mealPlans[0] || "EP" }] }))}>+ Add</button>
          </>
        )}
      </Card>
        ...
        {state.checkInRestrictions === "Applicable" && (
          <>
            {state.checkInRules.map((r) => (
              <div key={r.id} style={{ display: "flex", gap: 12, marginBottom: 8 }}>
                <input className="cc-input" placeholder="Check-in days" value={r.f1} onChange={(e) => setState((s) => ({ ...s, checkInRules: s.checkInRules.map((x) => x.id === r.id ? { ...x, f1: e.target.value } : x) }))} />
                <input className="cc-input" placeholder="Room type" value={r.f2} onChange={(e) => setState((s) => ({ ...s, checkInRules: s.checkInRules.map((x) => x.id === r.id ? { ...x, f2: e.target.value } : x) }))} />
                <input className="cc-input" placeholder="Condition" value={r.f3} onChange={(e) => setState((s) => ({ ...s, checkInRules: s.checkInRules.map((x) => x.id === r.id ? { ...x, f3: e.target.value } : x) }))} />
              </div>
            ))}
            <button className="cc-btn cc-btn-outline" onClick={() => setState((s) => ({ ...s, checkInRules: [...s.checkInRules, { id: uid(), f1: "", f2: "", f3: "" }] }))}>+ Add row</button>
          </>
        )}
      </Card>
      ...
      <Card title="Room inventory configuration">
        ...
        <table className="cc-table cc-table-compact">
          <thead><tr><th>Room type</th><th>PMS inventory</th><th>Rooms allocated</th><th>Release</th></tr></thead>
          <tbody>
            {state.rooms.map((r) => {
              ...
              return (
                <tr key={r.id}>
                  ...
                  <td><input className="cc-input" style={{ width: 100 }} value={inv.allocated} onChange={(e) => upd({ allocated: e.target.value })} /></td>
                  <td><input className="cc-input" style={{ width: 100 }} value={inv.release} onChange={(e) => upd({ release: e.target.value })} /></td>
                </tr>
              );
            })}
          </tbody>
      ...
      {/* Cancel modal */}
      <Modal open={cancelModal.open} onClose={() => setCancelModal({ ...cancelModal, open: false })}
        title={cancelModal.type === "before" ? "Before check-in — Cancellation policy" : "After check-in — Cancellation policy"}>
          ...
          {cancelModal.type === "before" && (
            <div>
              ...
              <div style={{ display: "flex", gap: 16, paddingTop: 12 }}>
                <label className="cc-radio-row"><input type="radio" name="ct" defaultChecked /> Days</label>
                <label className="cc-radio-row"><input type="radio" name="ct" /> Date based</label>
              </div>
            </div>
          )}
          <div><label className="cc-label">From</label><input className="cc-input" value={draftCancel.appliesWhen} onChange={(e) => setDraftCancel({ ...draftCancel, appliesWhen: e.target.value })} /></div>
          <div><label className="cc-label">To</label><input className="cc-input" /></div>
          <div><label className="cc-label">Penalty</label><input className="cc-input" value={draftCancel.penalty} onChange={(e) => setDraftCancel({ ...draftCancel, penalty: e.target.value })} /></div>
          <div><label className="cc-label">Processing fees</label><input className="cc-input" value={draftCancel.processingFees} onChange={(e) => setDraftCancel({ ...draftCancel, processingFees: e.target.value })} /></div>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24 }}>
          <button className="cc-btn cc-btn-outline">+ Add more</button>
      ...
      <Modal open={minlosModal} onClose={() => setMinlosModal(false)} title="Minimum Length of Stay (MinLOS)">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
          <div><label className="cc-label">Restriction type</label>
          ...
          <div><label className="cc-label">Applies to</label><input className="cc-input" value={draftMinlos.appliesTo} onChange={(e) => setDraftMinlos({ ...draftMinlos, appliesTo: e.target.value })} /></div>
          <div><label className="cc-label">Minimum nights</label><input className="cc-input" value={draftMinlos.minNights} onChange={(e) => setDraftMinlos({ ...draftMinlos, minNights: e.target.value })} /></div>
          <div><label className="cc-label">Room type</label>
